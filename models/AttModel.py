...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state,final=False):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(
            xt, fc_feats, att_feats, p_att_feats, state, att_masks,final)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        # p_fc_feats=p_fc_feats.repeat(batch_size,1,1)
        # p_att_feats = p_att_feats.repeat(batch_size, 1, 1)
        # pp_att_feats = pp_att_feats.repeat(batch_size, 1, 1)
        #p_att_masks = p_att_masks.repeat(batch_size, 1, 1)
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                if t == 0:  # input <bos>
                    ...
                    #it = torch.randint(0, 1256, (beam_size,)).cuda()
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                ...
                #it=torch.randint(0,1256,(batch_size,)).cuda()
            logprobs, state = self.get_logprobs_state(
                it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
    ...
    def forward(self, xt, img_fc, state):
        ...
        for L in range(self.num_layers):
            ...
            if not self.use_maxout:
                in_transform = F.tanh(all_input_sums.narrow(
