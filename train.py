...
def train(opt):
    ...
    if opt.start_from is not None:
        ...
        with open(os.path.join(opt.start_from, 'infos_' + '-best' + '.pkl'), 'rb') as f:
    ...
    # vit_model,variant=load_model('segm/save/')
    vit_model.load_state_dict(torch.load('best_rsicd.pth')['state_dict'],strict=False)
    # cnn_model = create_extractor("/root/PycharmProjects/vgg_vae_best_model.pth")  #VGG
    vit_model = vit_model.cuda()

    # if vars(opt).get('start_from', None) is not None:
    #     vit_model.load_state_dict(torch.load(os.path.join(opt.start_from, 'model-vit-best.pth')))
    #     print("load cnn model parameters from {}".format(os.path.join(opt.start_from, 'model-vit-best.pth')))
    ...
    #         {'params': module.parameters()} for module in vit_model.finetune_modules
    ...
    if vars(opt).get('start_from', None) is not None:
        ...
        if opt.finetune_cnn_after != -1:
            if os.path.isfile(os.path.join(opt.start_from, 'optimizer-vit-best.pth')):
                vit_optimizer.load_state_dict(torch.load(
                    os.path.join(opt.start_from, 'optimizer-vit-best.pth')))

    def save_checkpoint(model, vit_model, infos, optimizer, vit_optimizer, histories=None, append=''):
        if len(append) > 0:
            append = '-' + append
        ...
        # vit_checkpoint_path = os.path.join(
        #     opt.checkpoint_path, 'model-vit%s.pth' % (append))
        # torch.save(vit_model.state_dict(), vit_checkpoint_path)
        # print("vit model saved to {}".format(vit_checkpoint_path))
        ...
        if opt.finetune_cnn_after != -1 and epoch >= opt.finetune_cnn_after:
            vit_optimizer_path = os.path.join(
                opt.checkpoint_path, 'optimizer%s-vit.pth' % (append))
            torch.save(vit_optimizer.state_dict(), vit_optimizer_path)
        ...
        while True:
            if epoch_done:
                ...
                if opt.finetune_cnn_after == -1 or epoch < opt.finetune_cnn_after:
                    ...
                    # for module in vit_model.fixed_modules:
            ...
            if opt.finetune_cnn_after != -1 and epoch >= opt.finetune_cnn_after:
                utils.clip_gradient(vit_optimizer, opt.grad_clip)
                vit_optimizer.step()
            ...
            if (iteration % opt.save_checkpoint_every == 0):
                ...
                val_loss, predictions, lang_stats = eval_utils.eval_split(
                    vit_model, model, lw_model.crit, loader, eval_kwargs)
                ...
                save_checkpoint(model, vit_model, infos,
                                optimizer, vit_optimizer, histories)
                if opt.save_history_ckpt:
                    save_checkpoint(model, vit_model, infos, optimizer, vit_optimizer,
                                    append=str(iteration))

                if best_flag:
                    save_checkpoint(model, vit_model, infos,
                                    optimizer, vit_optimizer, append='best')
    ...
    # val_loss, predictions, lang_stats = eval_utils.eval_split(
    #     vit_model, model, lw_model.crit, loader, test_kwargs)
