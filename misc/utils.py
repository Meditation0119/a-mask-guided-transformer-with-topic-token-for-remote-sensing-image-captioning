...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, mask):
        ...
        res=torch.softmax(input,2)
        loss = -input.gather(2, target.unsqueeze(2)).squeeze(2) * mask
        loss = torch.sum(loss) / torch.sum(mask)

        return loss
class TopicMatchLoss(nn.Module):
    def __init__(self):
        super(TopicMatchLoss, self).__init__()

    def forward(self, input, topic, mask):
        # truncate to the same size
        target = topic[:, :input.size(1)]
        mask = mask[:, :input.size(1)]
        res=torch.softmax(input,2).argmax(2)
        topic=topic.argmax(1)[:,0]+1
        #print(topic)
        topic=topic.unsqueeze(1).repeat(1,17)
        match_res=(res==topic).long()
        match_res=torch.abs(-(match_res.sum(1)-1))
        match_loss=match_res.sum(0)//input.shape[0]
        print(match_loss)
